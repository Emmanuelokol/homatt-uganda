...
function showPage(name, el) {
  ...
  document.getElementById('pageTitle').textContent =
    { dashboard:'Dashboard', bookings:'Bookings', escalations:'Escalations',
      ...
      'shop-orders': 'Shop Orders', support: 'Support Tickets',
      feedback: 'Feedback & Ratings', emergency: 'Emergency Contacts',
      finance: 'Finance & Payouts' }[name] || name;
}
...
async function loadAllData() {
  ...
  // Load new live sections in parallel
  loadSupportTickets();
  loadFeedback();
...
const DEMO_TICKETS = [
  { id: 't1', category: 'technical', priority: 'high',   subject: 'App crashes on booking', contact_name: 'Sarah Nalwoga', contact_phone: '0701234567', status: 'open',        submitted_at: '2026-06-14T09:00:00Z' },
  { id: 't2', category: 'medical',   priority: 'urgent', subject: 'Wrong drugs dispensed',  contact_name: 'James Okello',  contact_phone: '0702345678', status: 'in_progress', submitted_at: '2026-06-13T14:30:00Z' },
  { id: 't3', category: 'billing',   priority: 'medium', subject: 'Payment not confirmed',  contact_name: 'Grace Atim',    contact_phone: '0703456789', status: 'resolved',    submitted_at: '2026-06-12T11:00:00Z' },
  { id: 't4', category: 'delivery',  priority: 'low',    subject: 'Rider was late',         contact_name: 'Peter Ssali',   contact_phone: '0704567890', status: 'closed',      submitted_at: '2026-06-11T08:00:00Z' },
];

let _allTickets = [];

async function loadSupportTickets() {
  if (isDemoMode) { renderSupportTickets(DEMO_TICKETS); return; }
  const { data } = await sb.from('support_tickets').select('*').order('submitted_at', { ascending: false }).limit(100);
  renderSupportTickets(data || DEMO_TICKETS);
}
...
function renderSupportTickets(tickets) {
  ...
  document.getElementById('badgeSupport').textContent = open;
}

function _renderTicketsTable(tickets) {
  const tb = document.getElementById('supportTable');
  if (!tickets.length) { tb.innerHTML = '<tr><td colspan="7" class="text-muted text-sm" style="text-align:center;padding:24px">No tickets</td></tr>'; return; }
  const priorityColor = { urgent:'badge-emergency', high:'badge-urgent', medium:'badge-confirmed', low:'' };
  const statusColor   = { open:'badge-pending', in_progress:'badge-progress', resolved:'badge-completed', closed:'' };
  tb.innerHTML = tickets.map(t => `
    <tr>
      <td class="text-sm">${t.category || '—'}</td>
      <td><span class="badge ${priorityColor[t.priority]||''}">${t.priority}</span></td>
      <td class="fw-600">${t.subject}</td>
      <td class="text-sm">${t.contact_name || '—'}<br><span class="text-muted">${t.contact_phone || ''}</span></td>
      <td><span class="badge ${statusColor[t.status]||''}">${(t.status||'').replace('_',' ')}</span></td>
      <td class="text-sm text-muted">${fmtDate(t.submitted_at)}</td>
      <td>
        ${t.status === 'open' ? `<button class="btn btn-ghost btn-sm" onclick="updateTicket('${t.id}','in_progress')">Take</button>` : ''}
        ${t.status === 'in_progress' ? `<button class="btn btn-ghost btn-sm" onclick="updateTicket('${t.id}','resolved')">Resolve</button>` : ''}
      </td>
    </tr>
  `).join('');
}
...
function filterTickets(q) {
  _renderTicketsTable(_allTickets.filter(t =>
    (t.subject||'').toLowerCase().includes(q.toLowerCase()) ||
    (t.contact_name||'').toLowerCase().includes(q.toLowerCase())
  ));
}

function filterTicketsByStatus(s, el) {
  document.querySelectorAll('#page-support .filter-chip').forEach(c => c.classList.remove('active'));
  el.classList.add('active');
  _renderTicketsTable(s === 'all' ? _allTickets : _allTickets.filter(t => t.status === s));
}

async function updateTicket(id, newStatus) {
  if (!isDemoMode) {
    await sb.from('support_tickets').update({ status: newStatus, updated_at: new Date().toISOString() }).eq('id', id);
  }
  const idx = _allTickets.findIndex(t => t.id === id);
  if (idx !== -1) _allTickets[idx].status = newStatus;
  _renderTicketsTable(_allTickets);
  document.getElementById('badgeSupport').textContent = _allTickets.filter(t => t.status === 'open' || t.status === 'in_progress').length;
}

// ── FEEDBACK & RATINGS ────────────────────────────────────────────────────────

const DEMO_FEEDBACK = [
  { id: 'f1', type: 'clinic',    score: 5, category: 'care_quality',    message: 'Doctor was very thorough', contact: '0701111111', status: 'reviewed', created_at: '2026-06-14T10:00:00Z' },
  { id: 'f2', type: 'platform',  score: 4, category: 'app_experience',  message: 'Easy to book, great app',  contact: '0702222222', status: 'new',      created_at: '2026-06-13T09:00:00Z' },
  { id: 'f3', type: 'pharmacy',  score: 3, category: 'speed',           message: 'Drugs took too long',      contact: '0703333333', status: 'new',      created_at: '2026-06-12T14:00:00Z' },
  { id: 'f4', type: 'platform',  score: 5, category: 'cost',            message: 'Very affordable',          contact: '',           status: 'actioned', created_at: '2026-06-11T08:00:00Z' },
];

let _allFeedback = [];

async function loadFeedback() {
  if (isDemoMode) { renderFeedback(DEMO_FEEDBACK); return; }
  const { data } = await sb.from('feedback_ratings').select('*').order('created_at', { ascending: false }).limit(100);
  renderFeedback(data || DEMO_FEEDBACK);
}

function renderFeedback(rows) {
  _allFeedback = rows;
  _renderFeedbackTable(rows);

  const avg = (type) => {
    const vals = rows.filter(r => r.type === type).map(r => r.score);
    return vals.length ? (vals.reduce((a, b) => a + b, 0) / vals.length).toFixed(1) : '—';
  };
  document.getElementById('avgClinicRating').textContent = avg('clinic') === '—' ? '—' : '⭐ ' + avg('clinic');
  document.getElementById('avgPharmRating').textContent  = avg('pharmacy') === '—' ? '—' : '⭐ ' + avg('pharmacy');
  document.getElementById('avgPlatRating').textContent   = avg('platform') === '—' ? '—' : '⭐ ' + avg('platform');
  document.getElementById('totalReviews').textContent    = rows.length;
}

function _renderFeedbackTable(rows) {
  const tb = document.getElementById('feedbackTable');
  if (!rows.length) { tb.innerHTML = '<tr><td colspan="7" class="text-muted text-sm" style="text-align:center;padding:24px">No feedback yet</td></tr>'; return; }
  const stars = n => '⭐'.repeat(n);
  const statusColor = { new: 'badge-pending', reviewed: 'badge-confirmed', actioned: 'badge-completed' };
  tb.innerHTML = rows.map(r => `
    <tr>
      <td class="fw-600">${stars(r.score)} ${r.score}/5</td>
      <td class="text-sm">${r.type}</td>
      <td class="text-sm text-muted">${(r.category||'—').replace('_',' ')}</td>
      <td class="text-sm">${r.message || '—'}</td>
      <td class="text-sm text-muted">${r.contact || '—'}</td>
...
function filterFeedback(type, el) {
  document.querySelectorAll('#page-feedback .filter-chip').forEach(c => c.classList.remove('active'));
  el.classList.add('active');
  _renderFeedbackTable(type === 'all' ? _allFeedback : _allFeedback.filter(r => r.type === type));
}

// ── EMERGENCY CONTACTS ────────────────────────────────────────────────────────

const DEMO_EMERGENCY = [
  { id: 'e1', name: 'Mary Nalwoga',   relationship: 'Mother',  phone: '+256700111111', is_primary: true,  created_at: '2026-06-01T08:00:00Z', profiles: { full_name: 'Sarah Nalwoga' } },
  { id: 'e2', name: 'John Okello',    relationship: 'Brother', phone: '+256700222222', is_primary: true,  created_at: '2026-06-02T09:00:00Z', profiles: { full_name: 'James Okello' } },
  { id: 'e3', name: 'Rita Atim',      relationship: 'Spouse',  phone: '+256700333333', is_primary: false, created_at: '2026-06-03T10:00:00Z', profiles: { full_name: 'Grace Atim' } },
];

let _allEmergency = [];

async function loadEmergencyContacts() {
  if (isDemoMode) { renderEmergencyContacts(DEMO_EMERGENCY); return; }
  const { data } = await sb.from('emergency_contacts')
    .select('*, profiles(full_name)')
    .order('created_at', { ascending: false }).limit(100);
  renderEmergencyContacts(data || DEMO_EMERGENCY);
}

function renderEmergencyContacts(rows) {
  _allEmergency = rows;
  _renderEmergencyTable(rows);
}

function _renderEmergencyTable(rows) {
  const tb = document.getElementById('emergencyTable');
  if (!rows.length) { tb.innerHTML = '<tr><td colspan="6" class="text-muted text-sm" style="text-align:center;padding:24px">No emergency contacts found</td></tr>'; return; }
  tb.innerHTML = rows.map(r => `
    <tr>
      <td class="fw-600">${r.name}</td>
      <td class="text-sm">${r.relationship || '—'}</td>
      <td class="text-sm">${r.phone}</td>
      <td>${r.is_primary ? '<span class="badge badge-completed">Primary</span>' : '<span class="text-muted text-sm">Secondary</span>'}</td>
      <td class="text-sm text-muted">${(r.profiles && r.profiles.full_name) || '—'}</td>
      <td class="text-sm text-muted">${fmtDate(r.created_at)}</td>
    </tr>
  `).join('');
}
...
function filterEmergency(q) {
  _renderEmergencyTable(_allEmergency.filter(r =>
...
async function loadFinance() {
  if (isDemoMode) { renderFinance(DEMO_SHOP_ORDERS); return; }
  const { data } = await sb.from('shop_orders').select('*').order('created_at', { ascending: false }).limit(200);
  renderFinance(data || DEMO_SHOP_ORDERS);
}

function renderFinance(orders) {
  document.getElementById('finTotalOrders').textContent   = orders.length;
  const total = orders.reduce((s, o) => s + (o.total_price || 0), 0);
  document.getElementById('finTotalRevenue').textContent  = fmtK(total) + ' UGX';
  document.getElementById('finPendingOrders').textContent = orders.filter(o => o.status === 'pending').length;

  const tb = document.getElementById('finOrdersTable');
  if (!orders.length) { tb.innerHTML = '<tr><td colspan="5" class="text-muted text-sm" style="text-align:center;padding:24px">No orders</td></tr>'; return; }
  tb.innerHTML = orders.slice(0, 50).map(o => `
    <tr>
      <td><span class="code-badge">${String(o.id).slice(0,8).toUpperCase()}</span></td>
      <td class="fw-600">${o.product_name}</td>
      <td class="fw-600 text-green">UGX ${(o.total_price||0).toLocaleString()}</td>
      <td>${shopOrderStatusBadge(o.status)}</td>
      <td class="text-sm text-muted">${fmtDate(o.created_at)}</td>
    </tr>
  `).join('');
