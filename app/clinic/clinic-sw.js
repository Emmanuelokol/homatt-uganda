 ...
 */
const CACHE = 'homatt-clinic-v133';
...
const SHELL = [
  ...
  'js/ucg-autofill.js?v=20260830a',
  'manifest.json',
...
function offlineFallbackResponse() {
  ...
  var script =
    ...
    'async function healOnline(){var okAny=false;try{var ks=(await caches.keys()).filter(function(k){return k.indexOf("homatt-clinic-")===0;});if(!ks.length)ks=["homatt-clinic-v133"];for(var i=0;i<ks.length;i++){var c=await caches.open(ks[i]);var cs=core();for(var j=0;j<cs.length;j++){try{var r=await fetch(cs[j],{cache:"reload"});if(r&&r.ok&&!r.redirected){await c.put(cs[j],r.clone());okAny=true;}}catch(e){}}}}catch(e){}return okAny;}' +
    'async function run(){' +
