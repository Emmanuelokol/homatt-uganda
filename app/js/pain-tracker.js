...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function runAnalysis() {
    ...
    const { data: logs } = await Promise.race([
      supabase.from('pain_logs').select('*').eq('user_id', userId).gte('log_date', ninetyDaysAgo.toISOString().split('T')[0]).order('log_date', { ascending: false }),
...
Respond ONLY with valid JSON:
{
  ...
  async function loadHistory() {
    ...
    const { data: logs } = await Promise.race([
      supabase.from('pain_logs').select('*').eq('user_id', userId).order('log_date', { ascending: false }).limit(20),
      new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), 10000)),
    ]).catch(() => ({ data: null }));
