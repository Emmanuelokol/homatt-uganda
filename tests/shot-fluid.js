...
const SB='https://kgkdiykzmqjougwzzewi.supabase.co',PORT=9102,ORIGIN='http://localhost:'+PORT;
...
(async()=>{
  ...
  await page.evaluate(async ()=>{
    ...
    const el=document.getElementById('confirmedDx'); el.value='Typhoid';
    el.dispatchEvent(new Event('input',{bubbles:true}));
    await new Promise(r=>setTimeout(r,600));
    document.getElementById('ucgOneTap').click();
    await new Promise(r=>setTimeout(r,5000));
    const ask=document.getElementById('ucgAsk');
    if(ask&&getComputedStyle(ask).display!=='none'){const h=ask.querySelector('[data-h]'); if(h) h.click(); await new Promise(r=>setTimeout(r,4000));}
    const g=[...document.querySelectorAll('.ucg-mg')].find(x=>/Drips/.test(x.textContent));
    if(g) g.scrollIntoView({block:'center'});
  });
  await page.waitForTimeout(600);
  await page.screenshot({path:OUT+'fluid-'+theme+'.png'});
  await page.close();
  }
