...
const SB='https://kgkdiykzmqjougwzzewi.supabase.co',PORT=9095,ORIGIN='http://localhost:'+PORT;
(async()=>{
  ...
  for (const pg of ['settings.html','guidelines.html']) {
    await page.goto(ORIGIN+'/clinic/'+pg); await page.waitForTimeout(7000);
    const c = await page.evaluate(()=>{
      const t=document.body.innerText; const out=[];
      t.split('\n').forEach(l=>{ if(/refer/i.test(l)) out.push(l.trim().slice(0,110)); });
      return out.slice(0,8);
    });
    console.log('--- '+pg); c.forEach(l=>console.log('   ', l));
  }
